            ...
            if msg.data:    # Checking if msg.data isn't empty
                ...
                # Update the frontend with the latest available data
                    if 'data' in data:
                        ...
                        # Get the length of the parsed_text_description list
                        n = len(data['parsed_text_description'])

                        # Loop through to write each page
                        for i in range(n):

                            # assign
                            page_text = data['parsed_text_description'][i]
                            caption = data['parsed_image_description'][i]
                            image = base64_to_image(data['illustrations'][i])
